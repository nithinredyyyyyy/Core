...
The repo now includes a separate live-stream extraction worker for cloud deployment:

- Worker entry: [server/streamWorker.js](/C:/Users/surak/core/server/streamWorker.js)
- Backend extraction routes: [server/streamExtraction.js](/C:/Users/surak/core/server/streamExtraction.js)
- Worker guide: [docs/stream-extraction-worker.md](/C:/Users/surak/core/docs/stream-extraction-worker.md)

Run locally:

```bash
npm run stream-worker:once
...
After that, Admin -> Stream Fetch -> Queue Fetch Session is enough. The cloud worker will pick queued sessions automatically.

For a fully free cloud setup, set:

```bash
STREAM_OCR_MODE=tesseract
```

and install `tesseract-ocr` on the worker VM.
