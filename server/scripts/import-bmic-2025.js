...
const ALIAS_TO_TEAM_NAME = new Map([
  ["DRX", "DRX"],
...
function resolveTeamName(broadcastName) {
  return ALIAS_TO_TEAM_NAME.get(broadcastName) || null;
}

function getStandingsForStage(stageName) {
  if (stageName !== "Grand Finals") return [];
  return tournament.stages[0].standings.map((entry) => ({
