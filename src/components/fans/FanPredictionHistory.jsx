...
export default function FanPredictionHistory({
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="truncate text-sm font-bold text-slate-900">
                    {prediction.tournament_name || "Tournament prediction"}
                  </p>
                ...
                <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1">
                  <PredictionStatusIcon
                    ...
                    points={prediction.awarded_points}
                  />
                  <span className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">
                    {prediction.status === "settled"
                      ? `${prediction.awarded_points || 0} pts`
                      : "Pending"}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-3 space-y-1.5 text-[12px] text-slate-500">
                <p>
                  ...
                  <span className="font-semibold text-slate-800">
                    {prediction.winner_team || "Not selected"}
                  </span>
                </p>
                <p>
                  Fragger:{" "}
                  <span className="font-semibold text-slate-800">
                    {prediction.top_fragger || "Not selected"}
                  </span>
                </p>
                <p>
                  Top 3:{" "}
                  <span className="font-semibold text-slate-800">
                    {Array.isArray(prediction.top_three) &&
                    prediction.top_three.length > 0
