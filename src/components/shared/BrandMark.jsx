...
import { BRAND_CONCEPTS } from "@/components/shared/brand-concepts";
  ...
  return (
    <svg viewBox="0 0 64 64" className={className} aria-hidden="true">
      ...
      <circle
        ...
        r="22"
        fill="none"
        stroke="url(#core-pulse-gradient)"
        strokeWidth="6"
        opacity="0.45"
      />
      <path
...
export function BrandWordmark({ className = "" }) {
  return (
    <span
      className={`font-heading text-sm font-black uppercase tracking-[0.28em] text-foreground ${className}`}
    >
      Core
    </span>
  );
