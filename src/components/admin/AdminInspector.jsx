      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Multiple team rows resolving to one organization key.
          </p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-primary">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Participant teams without a resolved shared logo.
          </p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-primary">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Participant phases that do not yet map to any stored match rows.
          </p>
        </div>
      </div>
    </>
...
function InspectorStructurePanels({
  participantPhaseGroups,
  matchesByStage,
  resultCoverage,
}) {
  return (
          ...
          <div className="mt-4 space-y-3">
            {matchesByStage.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No matches stored for this tournament yet.
              </p>
            ) : (
                ...
                >
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">
                    {entries
                      .map(
                        (entry) =>
                          `#${entry.match_number || "-"} ${entry.map || "Unknown map"} (${entry.status})`,
                      )
                      .join(" / ")}
                  </p>
                </div>
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        {unresolvedParticipants.length === 0 ? (
          <p className="mt-3 text-sm text-muted-foreground">
            All participant names resolve to a known team identity.
          </p>
        ) : (
          <div className="mt-3 space-y-2">
            {unresolvedParticipants.map((entry) => (
              <div
                key={`${entry.team}-${entry.placement}`}
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          {duplicateOrganizations.length === 0 ? (
            <p className="mt-3 text-sm text-muted-foreground">
              No duplicate organization rows found in the live teams table.
            </p>
          ) : (
            <div className="mt-3 space-y-2">
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {organization.entries
                      .map((entry) => `${entry.name} (${entry.tag})`)
                      .join(" / ")}
                  </p>
                </div>
              ))}
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          {missingLogos.length === 0 ? (
            <p className="mt-3 text-sm text-muted-foreground">
              Every participant team in this tournament resolves to a shared
              logo.
            </p>
          ) : (
            <div className="mt-3 space-y-2">
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          <h3 className="font-semibold">
            Participant Phases Without Match Wiring
          </h3>
          {phaseWithoutMatches.length === 0 ? (
            <p className="mt-3 text-sm text-muted-foreground">
              Every stored participant phase connects to at least one match
              stage/group label.
            </p>
          ) : (
            <div className="mt-3 space-y-2">
              {phaseWithoutMatches.map((entry) => (
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {entry.count} teams in this phase
                  </p>
                </div>
              ))}
            </div>
...
export default function AdminInspector() {
  ...
  const matchesByStage = useMemo(() => {
    return [
      ...groupBy(
        tournamentMatches,
        (match) =>
          `${match.stage || "Unknown stage"}${match.group_name ? ` / ${match.group_name}` : ""}`,
      ).entries(),
    ];
  }, [tournamentMatches]);
  ...
  const resultCoverage = useMemo(() => {
    return tournamentMatches.map((match) => {
      const rows = tournamentResults.filter(
  ...
  const duplicateOrganizations = useMemo(() => {
    const grouped = [
      ...groupBy(teams, (team) => getOrganizationMeta(team).key).entries(),
    ];
    return grouped
      .reduce((items, [key, entries]) => {
  ...
  const phaseWithoutMatches = useMemo(() => {
    return participantPhaseGroups
      .reduce((items, [phase, entries]) => {
        const normalizedPhase = String(phase || "").toLowerCase();
        const hasMatch = tournamentMatches.some((match) => {
          const combined =
            `${match.stage || ""}${match.group_name ? ` - ${match.group_name}` : ""}`.toLowerCase();
          return (
            combined === normalizedPhase ||
            (match.stage || "").toLowerCase() === normalizedPhase
          );
        });
        if (!hasMatch) items.push({ phase, count: entries.length });
        return items;
      }, []);
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Inspect live tournament payloads, group phases, match wiring, and
          result coverage from the backend.
        </p>
      </div>
      ...
      {selectedTournament ? (
        <>
          <InspectorMetricCards
            selectedTournament={selectedTournament}
            tournamentMatches={tournamentMatches}
            tournamentResults={tournamentResults}
            unresolvedParticipants={unresolvedParticipants}
            duplicateOrganizations={duplicateOrganizations}
            missingLogos={missingLogos}
            phaseWithoutMatches={phaseWithoutMatches}
          />
          <InspectorStructurePanels
            participantPhaseGroups={participantPhaseGroups}
            matchesByStage={matchesByStage}
            resultCoverage={resultCoverage}
          />
          <InspectorIssuePanels
            unresolvedParticipants={unresolvedParticipants}
            duplicateOrganizations={duplicateOrganizations}
            missingLogos={missingLogos}
            phaseWithoutMatches={phaseWithoutMatches}
