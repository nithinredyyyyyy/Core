...
import { isCurrentCircuitArticle } from "@/lib/currentCircuit";
...
function getArticleImagePosition(article) {
  ...
  if (
    /dplus kia win india - korea invitational 2023|inaugural india - korea/i.test(
      text,
    )
  ) {
...
function ArticleImage({
  ...
  const imageClass = isContain
    ? "object-contain"
    : `object-cover ${getArticleImagePosition(article)}`;
  ...
  return (
    ...
    >
      <div
        className={`flex size-full items-center justify-center ${framePadding}`}
      >
        <img
          src={imageConfig.src}
          alt={article.title}
          className={`size-full transition-transform duration-300 group-hover:scale-[1.02] ${imageClass}`}
        />
      </div>
    </div>
  );
...
function SignalCard({ label, value, detail, accent = "default" }) {
  ...
  return (
    <div className={`rounded-[24px] border p-5 shadow-sm ${accentClass}`}>
      ...
      <p className="mt-3 text-xl font-black uppercase tracking-[-0.04em] text-foreground">
        {value}
      </p>
...
function NewsCard({ article, index, compact = false }) {
  return (
    ...
    >
      {!compact ? <ArticleImage article={article} /> : null}
      ...
      <div className={compact ? "" : "p-5"}>
        ...
        <h3
          className={`mt-3 font-black uppercase tracking-[-0.03em] text-foreground ${compact ? "line-clamp-2 text-base" : "line-clamp-2 text-xl"}`}
        >
          {article.title}
        </h3>
        <p
          className={`mt-3 text-muted-foreground ${compact ? "line-clamp-3 text-sm leading-6" : "line-clamp-3 text-sm leading-7"}`}
        >
  ...
  return (
    <div>
      ...
      <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
        Tournament news, announcements, game updates, roster moves, and daily
        desk coverage in one cleaner editorial feed.
      </p>
    </div>
  );
}

function FeaturedNewsHero({
  featuredArticle,
  filteredCount,
  circuitCount,
  selectedCategory,
  ...
  return (
    <m.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="w-full">
      <Link
        to={`/news/${featuredArticle.id}`}
        className="block w-full overflow-hidden rounded-[32px] border border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.94),rgba(248,243,235,0.98))] text-left shadow-[0_28px_70px_rgba(15,23,42,0.08)] transition-all hover:-translate-y-1 hover:border-primary/30 dark:bg-[linear-gradient(180deg,rgba(15,23,42,0.92),rgba(17,24,39,0.98))]"
      >
        <div className="grid gap-6 p-6 md:px-8 md:py-8 lg:grid-cols-[1.02fr_0.98fr] lg:px-10">
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <h2 className="max-w-4xl text-[2.5rem] font-semibold uppercase leading-[0.92] tracking-[-0.06em] text-foreground md:text-[4rem]">
            ...
            <div className="flex flex-wrap gap-3 text-[11px] font-bold uppercase tracking-[0.18em] text-muted-foreground">
              ...
              {featuredArticle.priority ? (
                <span className="rounded-full border border-amber-500/20 bg-amber-500/10 px-3 py-1.5 shadow-sm text-amber-700 dark:text-amber-300">
                  {statusLabel(featuredArticle.priority)}
                </span>
              ) : null}
              {featuredArticle.verification_status ? (
                <span className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3 py-1.5 shadow-sm text-emerald-700 dark:text-emerald-300">
                  {statusLabel(featuredArticle.verification_status)}
                </span>
              ) : null}
              <span className="rounded-full border border-border/80 bg-card px-3 py-1.5 shadow-sm">
          ...
          <div className="grid gap-4 lg:grid-rows-[1fr_auto]">
            <div className="overflow-hidden rounded-[28px] border border-border/70 bg-card shadow-[0_18px_44px_rgba(15,23,42,0.08)]">
              <ArticleImage
                ...
                heightClass="min-h-[280px] h-full"
            ...
            <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
              ...
              <SignalCard
                ...
                detail="Active news desk category controlling the article stack."
              />
            </div>
          </div>
        </div>
...
function NewsCategoryFilter({ categories, selectedCategory, setSelectedCategory }) {
  return (
...
function SpotlightStoriesSection({ spotlightArticles }) {
  return (
    <LightPanel className="p-6 md:p-7">
      <div className="flex items-center justify-between gap-4">
        ...
        <Asterisk className="hidden size-5 text-primary md:block" />
      </div>
...
function DeskPostureSection({ featuredArticle, selectedCategory }) {
  return (
    <LightPanel className="p-6 md:p-7">
      <div className="flex items-center justify-between gap-4">
        <div>
        ...
        <Sparkles className="hidden size-5 text-primary md:block" />
      </div>
      ...
      <div className="mt-5 grid gap-4">
        <div className="rounded-[22px] border border-border/70 bg-background/75 p-4">
          ...
          <p className="mt-2 text-lg font-black uppercase tracking-[-0.03em] text-foreground">
            {isCurrentCircuitArticle(featuredArticle)
              ? "Current tournament coverage"
              : "Daily desk coverage"}
          </p>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            The lead story is simply the newest article inside the current
            filter.
          </p>
        </div>
        <div className="rounded-[22px] border border-border/70 bg-background/75 p-4">
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Use the desk filter to narrow the feed into the categories
            currently present in the article stack.
          </p>
        </div>
      </div>
    </LightPanel>
...
function NewsArchiveSection({ archiveArticles }) {
...
export default function News() {
  ...
  const { featuredArticle, spotlightArticles, archiveArticles, filtered } =
    useMemo(() => {
      const seen = new Set();
      ...
      const ordered = filteredArticles.toSorted((a, b) => {
  ...
  const circuitCount = useMemo(
    () => articles.filter(isCurrentCircuitArticle).length,
    [articles],
  );
    ...
    return (
      <div className="space-y-6">
        ...
        <EmptyState
          ...
          description="News articles will appear here."
        />
      </div>
    );
  }
    ...
    <LazyMotion features={domAnimation}>
      <div className="space-y-6">
        ...
        <section className="grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
          <SpotlightStoriesSection spotlightArticles={spotlightArticles} />
          <DeskPostureSection
            featuredArticle={featuredArticle}
            selectedCategory={selectedCategory}
          />
        </section>
        <NewsArchiveSection archiveArticles={archiveArticles} />
      </div>
    </LazyMotion>
  );
}
